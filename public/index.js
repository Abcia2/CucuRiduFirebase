...
auth.onAuthStateChanged((user) => {
  if (user) {
    ...
    const playerRef = firebase
      .database()
      .ref(`rooms/${GlobalRoomCode}/players/${uid}`);
    ...
    document.getElementById("output").textContent = `Benvenuto, ${playerName}!`; // Remove me from Firebase when I disconnect

    playerRef.onDisconnect().remove(); // Abilita i pulsanti
        ...
        if (snapshot.val() === false) {
          ...
          if (roomCode) {
            ...
            roomRef.parent
              .get()
              .then((snapshot) => {
                // Verifica se c'è un admin
                if (snapshot.exists() && snapshot.val().admin === uid) {
                  db.ref(`rooms/${roomCode}`)
                    .remove()
                    .then(() => {
                      console.log(
                        `Stanza ${roomCode} eliminata perché l'admin si è disconnesso.`
                      );
                    });
                }
              })
              .catch((error) =>
                console.error(
                  "Errore nel verificare la stanza o i giocatori",
                  error
                )
              );
          }
...
function createRoom() {
  ...
  roomRef
    ...
    .then(() => {
      console.log(
        `Stanza creata con codice: ${roomCode} - GlobalRoomCode: ${GlobalRoomCode}`
      );
      localStorage.setItem("roomCode", roomCode); // Salva il codice stanza localmente
      document.getElementById(
        "output"
      ).textContent = `Stanza creata! Codice: ${roomCode}`;
    })
    .catch((error) =>
      ...
      if (snapshot.exists()) {
        roomRef
          ...
          .then(() => {
            ...
            document.getElementById(
              "output"
            ).textContent = `Sei entrato nella stanza: ${roomCode}`;
          })
          .catch((error) =>
